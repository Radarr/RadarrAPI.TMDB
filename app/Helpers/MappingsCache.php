...
class MappingsCache
{
    ...
    public static function rememberQuery($query)
    {
        ...
        $cache_key = self::buildKeyFromQuery($query);
        ...
        foreach ($results as $mapping) {
            ...
            Cache::put(self::buildKeyFromQuery(['id' => $mapping['id']]), $mapping, $mappingsStorageDate);
        }
        ...
        foreach ($to_cache as $item) {
            Cache::put(self::buildKeyFromQuery($item[0]), $item[1], $mappingsStorageDate);
        }
    ...
    public static function updateMapping(Mapping $mapping)
    {
        ...
        self::updateMappingWithQuery($mapping, $possible_object_location);

        foreach ($possible_storage_locations as $key => $value) {
            self::updateMappingWithQuery($mapping, [$key => $value]);
            self::updateMappingWithQuery($mapping, [$key => $value, 'mapable_type' => $type]);
        }
    }
    ...
    private static function updateMappingWithQuery(Mapping $mapping, $query)
    {
        ...
        $key = self::buildKeyFromQuery($query);
        if (Cache::has($key)) {
