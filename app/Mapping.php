...
class Mapping extends Model
{
    ...
    public function vote($direction = 1)
    {
        ...
        if (Event::whereIn('type', [EventType::AddedMapping, EventType::ApproveMapping, EventType::DisapproveMapping])->where('mappings_id', '=', $this->id)->where('ip', '=', $ip)->whereBetween('date', [Carbon::now()->addDays(-1), Carbon::now()])->first()) {
            return;
        }
...
class TitleInfo extends Model
{
    ...
    protected $fillable = ['aka_title', 'aka_clean_title', 'language'];
